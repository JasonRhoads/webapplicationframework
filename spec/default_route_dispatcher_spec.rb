...
RSpec.describe Clarity do
  context "Default route dispatcher" do
  ...
  context "Default route dispatcher" do
    context "GET /Genie_Aladdin.png" do
      let(:app)      { Clarity.new }
      let(:env)      { { "REQUEST_METHOD" => "GET", "PATH_INFO" => "/Genie_Aladdin.png", "HTTP_COOKIE" =>"_clarity_session=v+CoJ6sR6q/c5zCXRdiVLUt3/qh4NhazDcnVV7ZlzsBbeduCjhoTCQ==" } }
      let(:response) { app.call(env) }
      let(:response_code)     { response[0] }

      it "works - invalid path /Genie_Aladdin.png returns a 404 HTTP response code" do
